...
import { Transition, Spring, animated, interpolate } from 'react-spring/renderprops'
import './style.css'
...
class Pagination extends React.Component {
    ...
    componentDidUpdate(prevProps){
        if(prevProps.down !== this.props.down && !this.props.down){
          console.log(this.props.delta);
            if(Math.abs(this.props.delta[0]) >= Math.abs(this.props.delta[1])){
              ...
              if (this.props.delta[0] < -50) this.setState({currentView: (currentView + 1 > 1) ? 1 : currentView + 1})
              else if(this.props.delta[0] > 50) this.setState({currentView: (currentView - 1 < 0) ? 0 : currentView - 1})
            }
        ...
        // if(Math.abs(this.props.delta[0]) < Math.abs(this.props.delta[1])){
        //   this.setState({scrollPosition: _.clamp(this.state.scrollPosition + this.props.delta[1], 0)});
        // }
  ...
  render() {
    ...
    const count = 20;
    const to = { x: down ? ((Math.abs(xDelta) > Math.abs(yDelta)) ? xDelta/10 + xPosition : xPosition) /*_.clamp(xDelta + xPosition, -50, 50)*/ : xPosition, y: 0/*down ? ((yDelta > xDelta) ? yDelta + yPosition : yPosition ) : yPosition */};
      ...
      <div className="gestures-main">
        <Spring native to={to} immediate={n => { return(down && n === 'x')}}>
          {(props) => {
              ...
              console.log(props.x); return(
            <div
              className={classes.scrollZone}>
              <animated.div className = {classes.animatedWrapper}
                style={{
                  transform: interpolate([x, y], (x, y) => { return `translate3d(${x - this.state.currentView * 240}px,0px,0)`;}),
                }}>
